...
from core.constants import MAX_BLOCK_SIZE_HEX
from core.helpers import timestamp

ACTIVE_NODES_DELTA = 60 * 60 * 24 * 7
INACTIVE_NODES_DELTA = 60 * 60 * 24 * 90
MAX_NODES_COUNT = 100
...
class NodesManager:
    ...
    def init():
        ...
        NodesManager.last_messages = NodesManager.db.get("last_messages") or {
            "https://denaro-node.gaetano.eu.org": timestamp()
        }
    ...
    def get_recent_nodes():
        ...
        return [
            item[0]
            for item in sorted(full_nodes.items(), key=lambda item: item[1], reverse=True)
    ...
    def get_propagate_nodes():
        ...
        active = sample(active_nodes, k=10) if len(active_nodes) > 10 else active_nodes
        zero = sample(zero_nodes, k=10) if len(zero_nodes) > 10 else zero_nodes
        return active + zero
...
class NodeInterface:
    ...
    async def request(self, path: str, data: dict = None, sender_node: str = ""):
        payload = data or {}
